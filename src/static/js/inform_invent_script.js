...
for (key in window.gears) {

    window.gears_item.push(window.gears[key]);

}

console.log(window.gears_item);
    ...
    btn.addEventListener("mousedown", async(event) => {
        ...
        if (btn.dataset.use === "false"){
            ...
            if (who === 1) {
                ...
                itm = window.gears_item.find(item => item.id === id);
            }

            let st = "name-" + btn.dataset.iid;
